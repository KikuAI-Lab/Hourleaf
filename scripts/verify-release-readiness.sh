...
release_version_count="$(grep -Fo 'MARKETING_VERSION = 1.0.3;' "$project_file" | wc -l | tr -d '[:space:]' || true)"
[[ "$release_version_count" == "6" ]] \
    || fail "all shipping targets must use marketing version 1.0.3"
release_build_count="$(grep -Fo 'CURRENT_PROJECT_VERSION = 14;' "$project_file" | wc -l | tr -d '[:space:]' || true)"
[[ "$release_build_count" == "6" ]] \
    || fail "all shipping targets must use build number 14"
