...
struct HourleafQuickSurfaceWidgetView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 7) {
            Label(HourleafQuickSurfaceCopy.widgetTitle, systemImage: "leaf.fill")
                .font(.headline)
                .lineLimit(1)
    ...
    private var readyContent: some View {
        ...
        case .absent:
            Label(HourleafQuickSurfaceCopy.totalsHidden, systemImage: "eye.slash")
                ...
                .lineLimit(2)
        case let .shown(_, serviceMinutes, creditMinutes):
            VStack(alignment: .leading, spacing: 4) {
                Text(HourleafQuickSurfaceCopy.thisMonth)
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                HStack(alignment: .top, spacing: 8) {
                    totalCell(
                        label: HourleafQuickSurfaceCopy.service,
    ...
    private var timerContent: some View {
        ...
        case .disabled:
            Label(HourleafQuickSurfaceCopy.timerOff, systemImage: "timer")
                .font(.caption)
                .foregroundStyle(.secondary)
                .lineLimit(1)
        case .idle:
            Label(HourleafQuickSurfaceCopy.timerReady, systemImage: "timer")
                .font(.caption)
                .lineLimit(1)
        case let .running(startedAtEpochSeconds):
            let startDate = Date(timeIntervalSince1970: startedAtEpochSeconds)
            let endDate = max(Date(), startDate).addingTimeInterval(24 * 60 * 60)
            HStack(spacing: 5) {
                Image(systemName: "timer")
                    .accessibilityHidden(true)
                VStack(alignment: .leading, spacing: 1) {
                    Text(HourleafQuickSurfaceCopy.timerRunning)
                        .font(.caption)
                        .lineLimit(1)
...
struct HourleafQuickSurfaceWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .description(HourleafQuickSurfaceCopy.thisMonth)
