...
export function ScreenshotEditor() {
  ...
  async function captureSlide(
    ...
    try {
      ...
      return await flattenPng(dataUrl);
    } finally {
  ...
  async function flattenPng(dataUrl: string) {
    const source = await fetch(dataUrl).then((response) => response.blob());
    const response = await fetch("/api/opaque-png", {
      method: "POST",
      body: source,
    });
    if (!response.ok) throw new Error("Opaque PNG conversion failed");
    const opaquePng = await response.blob();

    return await new Promise<string>((resolve, reject) => {
      const reader = new FileReader();
      reader.onerror = () => reject(new Error("Opaque PNG encoding failed"));
      reader.onload = () => resolve(String(reader.result));
      reader.readAsDataURL(opaquePng);
    });
  }
